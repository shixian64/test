...
class PatternRecognizer:
        ...
        for template_name, template_config in self.pattern_templates.items():
            ...
            if matches:
                ...
                patterns.append(Pattern(
                    ...
                    recommendations=self._get_template_recommendations(template_name)
                ))
        ...
        for normalized_msg, count in message_counts.items():
            if count >= threshold:
                ...
                patterns.append(Pattern(
                    ...
                    first_seen=datetime.now(),
                    last_seen=datetime.now(),
                    recommendations=self._get_frequency_recommendations(normalized_msg)
                ))
        ...
        try:
            ...
            for cluster_id, cluster_items in cluster_groups.items():
                if len(cluster_items) >= 2:  # At least 2 items in cluster
                    ...
                    pattern_id = self._generate_pattern_id('ml_cluster', cluster_messages)
                    ...
                    patterns.append(Pattern(
                        ...
                        first_seen=datetime.now(),
                        last_seen=datetime.now(),
                        recommendations=['Investigate clustered messages for common cause']
                    ))
    ...
    def _recognize_sequence_patterns(self, log_lines: List[str]) -> List[Pattern]:
        ...
        for seq_type, count in sequence_counts.items():
            if count >= 2:  # At least 2 occurrences
                examples = [seq['example'] for seq in sequences if seq['type'] == seq_type]
    ...
    def _check_sequence_pattern(self, sequence: List[Tuple[str, str, str]]) -> Optional[Dict[str, Any]]:
    ...
    def _find_common_terms(self, messages: List[str]) -> str:
        ...
        word_counts = Counter(word for word in all_words if word not in common_words and len(word) > 2)
    ...
    def _get_frequency_recommendations(self, message: str) -> List[str]:
        ...
        if 'error' in message.lower():
            ...
            return ['Review frequent message pattern', 'Consider reducing log verbosity']
    ...
    def cluster_patterns(self, patterns: List[Pattern]) -> List[PatternCluster]:
        ...
        try:
            ...
            for cluster_id, cluster_patterns in cluster_groups.items():
                ...
                max_severity = max(cluster_patterns, key=lambda p: ['low', 'medium', 'high', 'critical'].index(p.severity)).severity
                
                clusters.append(PatternCluster(
                    cluster_id=f"cluster_{cluster_id}",
                    patterns=cluster_patterns,
                    centroid_description=self._find_common_terms([p.description for p in cluster_patterns]),
