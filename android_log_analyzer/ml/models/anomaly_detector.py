...
class AnomalyDetector:
    ...
    def extract_features(
        self, log_lines: List[str], time_window: int = 60
    ) -> np.ndarray:
        """
    ...
    def _parse_log_metrics(self, log_lines: List[str]) -> List[LogMetrics]:
        ...
        for line in log_lines:
            ...
            if match:
                ...
                error_count = len(
                    re.findall(r"(error|exception|fail)", message, re.IGNORECASE)
                )
                warning_count = len(
                    re.findall(r"(warn|slow|timeout)", message, re.IGNORECASE)
                )
    ...
    def _aggregate_by_time_window(
        self, metrics: List[LogMetrics], window_seconds: int
    ) -> List[Dict[str, Any]]:
        """Aggregate metrics by time windows"""
        if not metrics:
            return []
        ...
        for metric in metrics:
            window_index = int(
                (metric.timestamp - start_time).total_seconds() // window_seconds
            )
        ...
        for window_metrics in windows.values():
            ...
            window_data = {
                ...
                "avg_message_length": np.mean(
                    [m.message_length for m in window_metrics]
                ),
    ...
    def _metrics_to_features(
        self, windowed_metrics: List[Dict[str, Any]]
    ) -> List[List[float]]:
    ...
    def _rule_based_detection(self, log_lines: List[str]) -> List[AnomalyResult]:
        ...
        metrics = self._parse_log_metrics(log_lines)
        windowed_metrics = self._aggregate_by_time_window(
            metrics, 60
        )  # 1-minute windows
