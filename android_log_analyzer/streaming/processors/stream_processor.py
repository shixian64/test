...
class StreamProcessor:
    ...
    def _check_patterns(self, log_entry: LogEntry, current_time: datetime):
        ...
        error_count = sum(
            1
            for ts, entry in self.window_logs
        ...
        if (
            ...
            or "fatal" in log_entry.message.lower()
        ):
            self._generate_alert(
                "application_crash",
                AlertLevel.CRITICAL,
    ...
    def _update_metrics(self):
        ...
        if self.window_logs:
            error_logs = sum(
                1 for _, entry in self.window_logs if entry.level in ["E", "F"]
            )
