...
class AlertManager:
    ...
    def process_alert(self, alert: Alert):
        ...
        try:
            ...
            if not escalation_rule:
                logger.warning(
                    f"No escalation rule found for alert level: {alert.level}"
                )
                return
    ...
    def _is_duplicate_alert(self, alert: Alert) -> bool:
        ...
        window_start = current_time - timedelta(
            seconds=self.config.deduplication_window
        )

        # Check recent alerts for similar content
        for recent_alert in reversed(
            ...
            if (
                ...
                and recent_alert.source == alert.source
                and recent_alert.id != alert.id
            ):
                return True
...
{json.dumps(alert.metadata, indent=2)}
    ...
    def _send_slack(self, alert: Alert, config: Dict[str, Any]):
        ...
        if not REQUESTS_AVAILABLE:
            logger.warning(
                "Slack functionality not available (missing requests library)"
            )
            return
    ...
    def _send_webhook(self, alert: Alert, config: Dict[str, Any]):
        ...
        if not REQUESTS_AVAILABLE:
            logger.warning(
                "Webhook functionality not available (missing requests library)"
            )
            return
